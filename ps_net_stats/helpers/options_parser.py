...
Description
====================
Start a Prometheus client that pulls blocked and contended heat events from a
PowerScale cluster. The script will report back 

To improve security, it is recommended to use a user that has restricted
permissions. The user needs to have the following RBAC privileges:
Read - ISI_PRIV_LOGIN_SSH (Required to connect over SSH)

The user also needs to either be root or have the privileges to execute the
isi_for_array command. The latter can be accomplished through the use of sudo
and modifying the sudoers file through the use of the visudo command.
The entry in the sudoers file should look like the following:
<user_name> ALL=(root) NOPASSWD: /usr/bin/isi_for_array -sX netstat -iW

Replace <user_name> with an actual user name.
...
Quickstart
====================
python3 ps_net_stats.py --config=<yaml_config_file>.yml
